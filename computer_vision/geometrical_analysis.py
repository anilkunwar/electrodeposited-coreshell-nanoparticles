...
if uploaded_files:
    ...
    scale_bar_nm = st.number_input("Enter scale bar length (nm):", min_value=1.0, value=20.0)  # Default to 20 nm from image
    scale_bar_coords = {}
    for i, img in enumerate(images):
        st.write(f"Click two points on the scale bar for Image {i+1}")
        img_pil = Image.fromarray(img)
        def get_coords(x, y):
            scale_bar_coords[i] = (x, y, scale_bar_coords.get(i, [None, None])[1]) if scale_bar_coords.get(i) else (x, y, None)
        img_with_click = st.image(img, use_column_width=True, output="canvas", on_click=get_coords)
        if st.button(f"Confirm Scale Bar Points for Image {i+1}"):
            if len(scale_bar_coords.get(i, [])) == 2:
                x1, y1, _ = scale_bar_coords[i][0]
                x2, y2, _ = scale_bar_coords[i][1]
                pixel_distance = np.sqrt((x2 - x1)**2 + (y2 - y1)**2)
                scale_factor = scale_bar_nm / pixel_distance  # nm/pixel
                st.write(f"Scale factor for Image {i+1}: {scale_factor:.2f} nm/pixel")
    ...
    if st.button("Analyze Images"):
        for idx, img in enumerate(images):
            if idx not in scale_bar_coords or len(scale_bar_coords[idx]) != 2:
                st.write(f"Please confirm scale bar points for Image {idx+1} first.")
                continue

            st.write(f"Processing Image {idx+1}...")
            x1, y1, _ = scale_bar_coords[idx][0]
            x2, y2, _ = scale_bar_coords[idx][1]
            pixel_distance = np.sqrt((x2 - x1)**2 + (y2 - y1)**2)
            scale_factor = scale_bar_nm / pixel_distance
            ...
            st.write(f"Coordinate System (nm) for Image {idx+1}:")import streamlit as st
import cv2
import numpy as np
import matplotlib.pyplot as plt
from PIL import Image
