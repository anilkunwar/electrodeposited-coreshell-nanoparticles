...
def custom_tokenizer(doc):
    hyphenated_phrases = ["shell-diameter", "core-diameter", "shell-thickness", "electric-resistivity", "thermal-stability", "deposition-time", "particle-size", "Ag-shell", "Cu-core"]
...
def categorize_terms(db_file, similarity_threshold=0.7, min_freq=5):
    try:
        ...
        categories = {
            "Dimensions": ["shell diameter", "core diameter", "shell thickness", "particle size"],
            "Electrical Properties": ["resistivity", "electric resistivity", "conductivity"],
            "Thermal Properties": ["thermal stability", "thermal conductivity", "heat capacity"],
            "Deposition": ["deposition time", "electroless deposition", "deposition rate"]
        }
...
def perform_ner_on_terms(db_file, selected_terms):
    try:
        ...
        valid_units = {
            "Dimensions": ["nm", "μm"],
            "Electrical Properties": ["Ω·m", "μΩ·cm"],
            "Thermal Properties": ["°C", "K"],
            "Deposition": ["min", "hr", "s"]
        }
        valid_ranges = {
            "Dimensions": [(1, 1000, "nm"), (0.001, 10, "μm")],
            "Electrical Properties": [(1e-8, 1e-5, "Ω·m"), (1, 100, "μΩ·cm")],
            "Thermal Properties": [(0, 500, "°C"), (273, 773, "K")],
            "Deposition": [(1, 60, "min"), (0.1, 5, "hr"), (10, 3600, "s")]
        }
        numerical_pattern = r"(\d+\.?\d*[eE]?-?\d*|\d+)\s*(nm|μm|um|Ω·m|ohm·m|μΩ·cm|microohm·cm|°C|Celsius|K|Kelvin|min|minute|hr|hour|s|second)"
                ...
                for span, orig_sent, nearby_sent in spans:
                    ...
                    if value_match:
                        try:
                            ...
                            if unit in ["UM", "MICROMETER"]:
                                unit = "μm"
                            elif unit in ["NANOMETER"]:
                                unit = "nm"
                            elif unit in ["OHM·M"]:
                                unit = "Ω·m"
                            elif unit in ["MICROOHM·CM"]:
                                unit = "μΩ·cm"
...
db_files = [f for f in os.listdir(DB_DIR) if f.endswith('.db') and f in ["coreshellnanoparticles_metadata.db", "coreshellnanoparticles_universe.db"]]
db_options = db_files + ["Upload a new .db file"]
default_index = db_options.index("coreshellnanoparticles_universe.db") if "coreshellnanoparticles_universe.db" in db_options else 0
                ...
                if os.path.exists(st.session_state.db_file):
                    with st.spinner("Building knowledge graph..."):
                        ...
                        if G and G.edges():
                            ...
                            ax.set_title(f"Knowledge Graph of Core-Shell Nanoparticle Properties (Top {top_n} Terms per Category)")
                            plt.tight_layout()
...
- Check `coreshell_analysis.log` for detailed logs (in the same directory as the script).
""")
